...
class modelDataMaster extends CI_Model {
  ...
  public function saveRegion($args)
  {
      $this->db->set("region_name", $args->region_name);
      $this->db->set("description", $args->description);

      if(!empty($args->id)) {
          $this->db->where("id", $args->id);
          $this->valid = $this->db->update("region");
      }
      else{
          $this->valid = $this->db->insert("region");
      }
	...
	public function saveRetailer($args)
  {
      ...
      $this->db->set("address", $args->address);
      $this->db->set("region", $args->regionhidden);
	  $this->db->set("id_region", $args->id_region);
	  
	  if(!empty($args->place_birth)) {
		echo('alert($args->place_birth)');
		$this->db->set("place_birth", $args->place_birth);
	  }
