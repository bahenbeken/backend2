...
class Utility extends MX_Controller {
	...
	public function user($id = NULL)
	{
		...
		if(!empty($id))
		{
			$sql = "select a.*, b.name as retailer_name, c.region_name from usertable a left join retailtable b on b.id=a.retail_id left join region c on c.id=a.id_region where a.id='".$id."'";
			$this->container["edit"] = $this->db->query($sql)->row();
		}

		$sqlData = "select a.*, b.name as retailer_name, c.region_name from usertable a left join retailtable b on b.id=a.retail_id left join region c on c.id=a.id_region";
