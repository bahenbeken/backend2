...
class Login extends MX_Controller {
	...
	public function resetPassword($userTable, $userId)
	{
		...
		if($_POST) {
			...
			$this->db->set("forgot_password", "");
			$this->db->where("id", $args->user_id);
			$this->valid = $this->db->update($args->user_table);

			if($this->valid) {
				$this->session->set_flashdata(array("type" => "success", "notify" => "Reset password success! you can login now"));
			}
		}

		$this->container["user_table"] = $userTable;
		$this->container["user_id"] = $userId;

		$this->twig->display("form/formResetPassword.html", $this->container);
	}
